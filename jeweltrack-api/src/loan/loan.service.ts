...
export class LoanService {
  ...
  // 2. List All Loans with Calculated Repayments & Outstanding Balances
  async getLoans(shopId: string, status?: LoanStatus, search?: string) {
    ...
    return loans.map((loan) => {
      const totalPrincipalPaid = loan.loanRepayment.reduce((sum, r) => sum + (r.principal_paid || 0), 0);
      const totalInterestPaid = loan.loanRepayment.reduce((sum, r) => sum + (r.interest_paid || 0), 0);
      const currentPrincipalBalance = Math.max(0, loan.loan_amount - totalPrincipalPaid);

      // Calculate accrued days since loan creation
      const loanDate = new Date(loan.loan_date);
      const diffMs = now.getTime() - loanDate.getTime();
      ...
      const totalEstimatedInterest = Math.round(monthlyInterestAmount * monthsElapsed);
      const pendingInterest = Math.max(0, totalEstimatedInterest - totalInterestPaid);
  ...
  // 4. Record Interest / Principal Repayment or Settle Loan
  async repayLoan(shopId: string, loanId: string, dto: RepayLoanDto) {
    ...
    await this.prisma.loanRepayment.create({
      data: {
        loan_id: loanId,
        principal_paid: Number(dto.principal_paid) || 0,
        interest_paid: Number(dto.interest_paid) || 0,
    ...
    const totalPrincipalPaid =
      loan.loanRepayment.reduce((sum, r) => sum + (r.principal_paid || 0), 0) + (Number(dto.principal_paid) || 0);

    const shouldClose = dto.is_closing || totalPrincipalPaid >= loan.loan_amount;
